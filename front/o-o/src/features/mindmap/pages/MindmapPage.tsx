...
const MindmapPageContent: React.FC = () => {
  ...
  useEffect(() => {
    ...
    console.log("🎨 [MindmapPage] Setting initial awareness state:", initialState);
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (event: cytoscape.EventObject) => {
      ...
      raf = requestAnimationFrame(() => {
        ...
        if (Date.now() - lastLog > 5000) {
          console.log("🖱️ [MindmapPage] Setting cursor (model coords):", cursorData);
          lastLog = Date.now();
        }
    ...
    console.log("🎮 [MindmapPage] Attaching mousemove to cy");
