...
class HybridVisionAnalyzer:
    ...
    def _load_yolo(self):
        ...
        try:
            ...
            print("✅ YOLO v11 nano 로드 완료 (VRAM ~1GB)")
    ...
    def _load_clip(self):
        ...
        try:
            ...
            print("✅ CLIP ViT-B/32 로드 완료 (VRAM ~1GB)")
        except Exception as e:
    ...
    def _load_ocr(self):
        ...
        try:
            ...
            print("✅ PaddleOCR 로드 완료 (VRAM ~500MB)")
        except Exception as e:
            print(f"⚠️  OCR 로드 실패: {e}")
            print("   설치: pip install paddleocr paddlepaddle-gpu")
    ...
    def _load_llm(self, quantization: str = "int4"):
        ...
        try:
            ...
            print("✅ Llama 3.1 8B 로드 완료")
        except Exception as e:
            print(f"⚠️  Llama 로드 실패: {e}")
...
def test_hybrid_analysis(
    ...
    """
    print("=" * 80)
    print("🎬 하이브리드 YouTube 영상 분석 시작")
    print("=" * 80)
    print(f"📺 URL: {youtube_url}")
    print(f"🖼️  최대 프레임: {max_frames}개")
    print(f"🦙 LLM 양자화: {llm_quantization}")
    print()
    ...
    if transcript_result['success']:
        ...
        print(f"\n📄 자막 미리보기 (처음 300자):")
        print("-" * 80)
        print(transcript_result['full_text'][:300] + "...")
        print("-" * 80)
    else:
    ...
    for i, frame in enumerate(frames, 1):
        ...
        # 결과 미리보기
        if result['scene']:
            print(f"   장면: {result['scene'][0]['scene']} ({result['scene'][0]['confidence']:.1%})")
        if result['objects']:
            obj_names = [obj['name'] for obj in result['objects'][:5]]
            print(f"   객체: {', '.join(obj_names)}")
        if result['text']:
            ocr_texts = [t['text'] for t in result['text'][:2]]
            print(f"   텍스트: {' / '.join(ocr_texts)}")

    # Step 5: LLM 종합 분석
    print("\n" + "=" * 80)
    print("🧠 STEP 5: LLM 종합 분석 및 요약")
    print("=" * 80)
...
필수 환경변수 (.env):
    ...
    if len(sys.argv) < 2:
        print("\n❌ YouTube URL을 입력해주세요!")
        print("예시: python test_youtube_analyzer_hybrid.py 'https://www.youtube.com/watch?v=dQw4w9WgXcQ'\n")
        sys.exit(1)
